...
exports.AgentControllerImpl = void 0;
const ContextProcessor_1 = require("./ContextProcessor");
const services_1 = require("../services");
const AIService_1 = require("../services/AIService");
class AgentControllerImpl {
    constructor(knowledgeEngine, contextProcessor, aiService) {
        this.knowledgeEngine = knowledgeEngine;
        this.scopeValidator = new services_1.ScopeValidator();
        this.responseFormatter = new services_1.ResponseFormatter();
        this.contextProcessor = contextProcessor || new ContextProcessor_1.ContextProcessorImpl();
        this.aiService = aiService || (0, AIService_1.createAIService)();
    ...
    async processQuery(query, context) {
        try {
            ...
            if (!query || query.trim().length === 0) {
                return this.responseFormatter.formatErrorResponse('Please provide a phrase or expression to translate.', 'EMPTY_QUERY', ['Try asking about a specific Tamil phrase you heard', 'Include context about where you heard it']);
            }
            // Check if system is initialized
            if (!this.knowledgeEngine.isInitialized()) {
                return this.responseFormatter.formatErrorResponse('TalkMate is not properly initialized. Please try again.', 'NOT_INITIALIZED', ['Wait a moment and try again', 'Contact support if the problem persists']);
            }
            // Validate scope
            ...
            return this.responseFormatter.formatErrorResponse('An error occurred while processing your query. Please try again.', 'PROCESSING_ERROR', ['Try rephrasing your question', 'Check if your query is about Thanjavur']);
        }
    }
    ...
    async generateCulturalResponse(query, context) {
        try {
            ...
            if (matches.length > 0) {
                ...
                // Format the response
                const response = this.responseFormatter.formatSuccessResponse(matches, query, additionalContext, additionalGuidance);
    ...
    enrichContextFromQuery(query, providedContext) {
    ...
    getAdditionalCulturalContext(context) {
        const contextParts = [];
        if (context.speaker) {
    ...
    getAdditionalVisitorGuidance(context) {
        const guidanceParts = [];
        if (context.speaker) {
            switch (context.speaker) {
                case 'shopkeeper':
...
exports.AgentControllerImpl = AgentControllerImpl;
