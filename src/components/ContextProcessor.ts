...
export class ContextProcessorImpl implements ContextProcessor {
  private speakerKeywords: Map<SpeakerType, string[]> = new Map([
  ...
  private locationKeywords: Map<LocationType, string[]> = new Map([
  ...
  enrichQuery(query: string, context: Context): EnrichedQuery {
    ...
    if (context.speaker) confidence += 0.2;
    if (context.location) confidence += 0.2;
    if (context.additionalInfo) confidence += 0.1;
    
    // Boost confidence for clear language detection
    if (detectedLanguage !== 'mixed') confidence += 0.1;
  ...
  private countTamilCharacters(text: string): number {
    let count = 0;
