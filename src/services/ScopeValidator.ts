...
export class ScopeValidator {
  ...
  validateThanjavurScope(query: string): { isValid: boolean; reason?: string } {
    ...
    const hasGenericReference = this.genericTamilKeywords.some(keyword => 
      normalizedQuery.includes(keyword)
    );

    const hasThanjavurReference = this.thanjavurKeywords.some(keyword => 
      normalizedQuery.includes(keyword)
    );
